...
txt  = open(path).read()
old  = '<button class="btn-amber" id="btnAuto" onclick="autoScrape()" style="display:none">⚡  Auto JSON</button>'
new  = old + '\n    <button class="btn-gray" id="btnDeep" onclick="showDeepPanel()" style="display:none">🔗 Deep</button>'
if "btnDeep" in txt:
    print("btnDeep déjà présent, skip")
elif old not in txt:
    print("ERREUR : ancre btnAuto introuvable")
    raise SystemExit(1)
else:
